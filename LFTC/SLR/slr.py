...
def main():
	...
	for keys, values in goto_dict.items():
		for value in values:
			value = value.split(',')
			if value[0] not in non_terminals:
	...
	table = [[0 for i in range(len(action_set) + len(non_terminals) + 1)] for j in range(no_items + 1)]
	table[0][0] = 'State'

	for i in range(1, len(action_set) + 1):
		table[0][i] = action_set[i - 1]
		hash_all[action_set[i - 1]] = i

	for j in range(i + 1, i + len(non_terminals) + 1):
	...
	input_string2 = sys.argv[2]
	input_stack = list(input_string2)
	input_stack.append('$')
